...
export function Header() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <header style={{backgroundColor: '#1E5A40'}} className="absolute inset-x-0 top-0 z-50">
        <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <a style={{color:'white'}} key={item.name} href={item.href} className="text-sm/6 font-semibold text-gray-900">
                {item.name}
              </a>
            ))}
          </div>
        ...
        <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
          ...
          <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white p-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            <div className="flex items-center justify-between">
              <a href="#" className="-m-1.5 p-1.5">
                <span className="sr-only">Your Company</span>
                <h1 className="text-3xl font-bold tracking-[0.45rem] uppercase text-gray-900 transition-all duration-300 hover:text-emerald-200 hover:drop-shadow-lg">PhysaFlow</h1>
              </a>
              <button
                type="button"
