      ...
      <nav aria-label="Global" className="py-6">
        ...
        <div className="flex lg:flex-1">
          <a href="#" className="-m-1.5 p-1.5">
            ...
            <h1 className="text-3xl font-bold tracking-[0.45rem] uppercase text-white transition-all duration-300 hover:text-emerald-200 hover:drop-shadow-lg">
              PhysaFlow
            </h1>
          </a>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-sm/6 font-semibold text-white transition-colors hover:text-emerald-200"
            >
              {item.name}
            </a>
          ))}
      ...
      >
        ...
        <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white p-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
          <div className="flex items-center justify-between">
            <a href="#" className="-m-1.5 p-1.5">
              <span className="sr-only">PhysaFlow</span>
              <h1 className="text-3xl font-bold tracking-[0.45rem] uppercase text-gray-900">
                PhysaFlow
              </h1>
            </a>
            <button
